const createVendorCodeMetricsSingle = (vendorCodeMetrics) => {
  const vcMetrics = vendorCodeMetrics.map((item) => {
    ...
    return {
      ...
      abcCurrent: item.abc.abc_history.split(',').slice(-1),
      abcCtgryCurrent: item.abc.abc_in_ctgry_history.split(',').slice(-1),
      tags: item.tags,
    };
  });
  return vcMetrics;
...
export default createVendorCodeMetricsSingle;
