...
import styles from './ChartGanttRow.module.css';
...
const ChartGanttRow = ({
  ...
  const sizeRows = useMemo(
    () =>
      sku.sizes.map((chart) => {
        const cells = buildCells(chart, weeks, extraOrders, startCalcDate);
        const ordersInBD = Math.round(
          chart.orders.reduce((a, o) => a + o.quantity, 0)
        );
        return { chart, cells, ordersInBD };
      }),
    [sku.sizes, weeks, extraOrders, startCalcDate]
  );
  ...
  const totals = useMemo(() => {
    let sO = 0,
      sD = 0,
      sE = 0;
    sizeRows.forEach(({ chart, cells, ordersInBD }) => {
      sO += ordersInBD;
      sE += extraOrders[chart.chartId] || 0;
      const last = cells[cells.length - 1];
      if (last?.stock < 0) sD += -last.stock;
    });
    return { sO, sD, sE };
  }, [sizeRows, extraOrders]);
  ...
  return (
    <div className={styles.tableWrapper}>
      ...
      <table className={styles.ganttTable}>
        ...
        <tbody>
          {sizeRows.map(({ chart, cells, ordersInBD }) => {
            const last = cells[cells.length - 1];
            const deficit = last?.stock < 0 ? -last.stock : 0;
            return (
              <tr key={chart.chartId}>
                <td className={styles.colSize}>{chart.size}</td>
                ...
                <td className={styles.colTurnover}>
                  {chart.turnoverTotal
                    ? Math.round(chart.turnoverTotal)
                    : <span className={styles.dash}>—</span>}
                </td>
                <td className={styles.colNewOrder}>
                  <input
                    type="number"
                    value={extraOrders[chart.chartId] || ''}
                    onChange={(e) =>
                      onExtraOrderChange(chart.chartId, e.target.value)
                    }
                    className={styles.extraOrderInput}
                  />
                </td>
                {cells.map((cell, i) => {
