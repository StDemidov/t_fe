...
const BodyTags = ({ tags, availableTagsMain, skuId, tableRef }) => {
  ...
  const handleAddTag = useCallback(() => {
    ...
    let check = availableTagsMain.map((e) => {
      return e.toLowerCase();
    });

    if (newTag && !check.includes(newTag.toLowerCase())) {
      dispatch(setNewAvailableTagMain([newTag]));
  ...
  const handleAccept = () => {
    ...
    dispatch(setNewSkusTagsMain({ [skuId]: tempTags }));
    setIsDropdownOpen(false);
  };
