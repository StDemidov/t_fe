...
const ButtonRestartCamps = ({ selectedCamps }) => {
  ...
  const handleClickOnPause = (event) => {
    ...
    openConfirmModal(
      `Вы уверены, что хотите перезапустить кампании (${selectedCamps.length} шт.)?`,
      (e) => {
        dispatch(
          changeCampaignsStatuses({
            data: data,
            url: `${hostName}/ad_camps/change_status/${-12}`,
          })
        );
      }
    );
  ...
  return (
    <>
      ...
      >
        <FaPlayCircle className={styles.gear} />
      </button>
      <ConfirmModal
...
export default ButtonRestartCamps;
